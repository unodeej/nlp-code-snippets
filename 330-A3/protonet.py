...
class ProtoNet:
    ...
    def _step(self, task_batch):
        ...
        for task in task_batch:
            ...
            ### START CODE HERE ###
            num_support = labels_support.tolist().count(labels_support[0])
            num_query = labels_query.tolist().count(labels_query[0])
            num_ways = len({ i for i in labels_support.tolist() })

            with torch.no_grad():
                latents_support = self._network(images_support)
            
            latents_query = self._network(images_query)

            prototypes_support = []
            prototypes_query = []
            for way in range(num_ways):
                start = num_support * way
                end = num_support * (way + 1)
                latents_for_way = latents_support[start:end, :]
                p = torch.mean(latents_for_way, dim=0)
                p = p.reshape(1, p.shape[0])
                p_support = p.repeat(latents_support.shape[0], 1)
                p_query = p.repeat(latents_query.shape[0], 1)
                prototypes_support.append(p_support)
                prototypes_query.append(p_query)

            prototypes_support = torch.stack(prototypes_support)
            prototypes_query = torch.stack(prototypes_query)

            # print(prototypes_support.shape)
            # print(prototypes_query.shape)

            logits_support = []
            logits_query = []
            for i in range(prototypes_support.shape[0]):
                p = prototypes_support[i, :, :]
                logits_support.append(-(torch.sqrt(torch.sum(torch.pow(torch.subtract(latents_support, p), 2), dim=1)) ** 2))
            
            for i in range(prototypes_query.shape[0]):
                p = prototypes_query[i, :, :]
                logits_query.append(-(torch.sqrt(torch.sum(torch.pow(torch.subtract(latents_query, p), 2), dim=1)) ** 2))

            logits_support = torch.stack(logits_support)
            logits_query = torch.stack(logits_query)

            # print(logits_support.shape)
            # print(logits_query.shape)

            predictions_support = torch.argmax(logits_support, dim=0)
            predictions_query = torch.argmax(logits_query, dim=0)

            # print(predictions_support.shape)
            # print(predictions_query.shape)

            

            accuracy_support = 0
            for i in range(predictions_support.shape[0]):
                if predictions_support[i] == labels_support[i]:
                    accuracy_support += 1
            accuracy_support = accuracy_support / predictions_support.shape[0]
            accuracy_support_batch.append(accuracy_support)

            accuracy_query = 0
            for i in range(predictions_query.shape[0]):
                if predictions_query[i] == labels_query[i]:
                    accuracy_query += 1
            accuracy_query = accuracy_query / predictions_query.shape[0]
            accuracy_query_batch.append(accuracy_query)

            # print(accuracy_support)
            # print(accuracy_query)

            logits_query = torch.transpose(logits_query, 0, 1)
            
            loss_fn = nn.CrossEntropyLoss()
            res = loss_fn(logits_query, labels_query)
            # print(res)
            loss_batch.append(res)
            ### END CODE HERE ###
        return (
