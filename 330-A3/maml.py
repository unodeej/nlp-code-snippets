...
class MAML:
    ...
    def _inner_loop(self, images, labels, train):
        ...
        for _ in range(self._num_inner_steps):
            ...
            score = util.score(logits, labels)
            ...
            gradients = autograd.grad(outputs=loss, inputs=inputs, create_graph=train)

            parameters = {
                k: parameters[k] - self._inner_lrs[k] * gradients[i]
    ...
    def _outer_step(self, task_batch, train):
        ...
        for task in task_batch:
            ...
            ### START CODE HERE ###
            params, accs, _ = self._inner_loop(images_support, labels_support, train)
            logits = self._forward(images_query, params)

            loss = nn.functional.cross_entropy(logits, labels_query)

            score = util.score(logits, labels_query)

            outer_loss_batch.append(loss)
            accuracies_support_batch.append(accs)
            accuracy_query_batch.append(score)
            ### END CODE HERE ###
