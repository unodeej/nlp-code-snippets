    ...
    **Note**: Be sure to shuffle the support examples and labels 
    ...
    if len(support_inputs) > 1:
        ...
        for i in np.nditer(p):
            inputs.append(support_inputs[i])
            labels.append(support_labels[i])
        support_inputs = inputs
        support_labels = labels

    if prompt_mode == 'none':
        for i in range(len(support_inputs)):
            prompt += support_inputs[i] + ' ' + support_labels[i] + ' '
        prompt += test_input
    elif prompt_mode == 'tldr':
        for i in range(len(support_inputs)):
            prompt += support_inputs[i] + ' TL;DR: ' + support_labels[i] + '. '
        prompt += test_input + ' TL;DR:'
    elif prompt_mode == 'qa':
        for i in range(len(support_inputs)):
            prompt += support_inputs[i] + ' In the ' + support_labels[i] + '. '
        prompt += test_input + ' In the'
    elif prompt_mode == 'custom':
        for i in range(len(support_inputs)):
            prompt += support_inputs[i] + ' Location: ' + support_labels[i] + '. '
        prompt += test_input + ' Location:'
    print(prompt)
    print('.')
    ### END CODE HERE ###
...
def do_sample(model, input_ids, stop_tokens, max_tokens):
    ...
    while (len(sampled_tokens) < max_tokens):
        ...
        token = torch.argmax(res.logits[0][res.logits.shape[1] - 1])
        if token in stop_tokens:
            break
        sampled_tokens.append(token)
        input_ids = torch.cat((input_ids, token.reshape(1, 1)), 1)
    ### END CODE HERE ###
                ...
                for k in ks:
                    ...
                    for repeat in range(repeats):
                        ...
                        for row in pbar:
                            ...
                            ### START CODE HERE ###
                            input_string = get_icl_prompts(support_x, support_y, test_input)
                            # be = tokenizer(input_string, return_tensors='pt').to('mps')
                            be = tokenizer(input_string)
                            input_ids = torch.IntTensor(be['input_ids'])
                            input_ids = torch.reshape(be['input_ids'], (1, be['input_ids'].shape[0]))
                            sampled_tokens = do_sample(model, input_ids, stop_tokens, max_tokens)
                            # sampled_tokens = do_sample(model, be['input_ids'], stop_tokens, max_tokens)
